...
## Colour roles
- **Accent presets (the student asked for a choice of two-colour accents):** Settings > Appearance > Accent colours offers Blue + Gold (default, the student's choice), Mono, Navy + Coral, Ink + Tangerine. Hover: the cursor spotlight from Jen's landing page, drawn in pixels (src/ui/effects/spotlight.ts + src/styles/spotlight.css): a hovered card lights up as a dot matrix, --accent-2 at the pointer stepping out through --accent in whole levels, with its outer row of pixels as a brighter rim, switching on in four steps. Add new card-like surfaces to its selector list (a test keeps the stylesheet's and the script's lists the same). Stored as `settings.accent`, applied as `<html data-accent>`. Every screen must use `--accent` for the primary button (and `--accent-soft` / `--accent-line` for selected tints) and `--accent-2` (via `--progress`) for progress bars and "you are here" markers, so all presets work automatically. Never hard-code accent colours.
...
## Other screens
Same frame and language: topic page (title, progress card, tabs with ink underline, question rows like the ladder, minimal filters), Playground (full-height dark editor card with file tabs, Run, bottom output panel), exams (a segmented choice of mock final paper or custom practice test; the mock paper lists its eight slots with marks and the topic each is drawn from, never the question titles, which would spoil it; running test with dots + mono timer; results cards, marked out of 100 for a mock paper), reports (stat numbers, strengths / work-on-next cards, grey heat table), settings (editor-style list).
