...
const questions: StatQuestion[] = [
  {
    ...
    kind: 'choice',
    marks: 2,
    prompt: 'Read Theta and its standard error at the bottom of this summary. What does a large Theta tell you about these counts, compared with a Poisson model?',
    code: 'library(MASS)\nnb <- glm.nb(Days ~ Eth + Sex + Age + Lrn, data = quine)\nsummary(nb)\n',
    options: [
      { text: 'They are close to Poisson: the extra variance term, mean squared over theta, is small next to the mean', correct: true },
      { text: 'The counts are very spread out' },
    ...
    explain: 'The negative binomial variance is mean + mean-squared / theta. Theta sits under the squared mean in the extra term, so the larger it is, the less extra spread it adds; as theta grows without end the model becomes the Poisson. A small theta is what large extra spread looks like, and theta is a separate quantity from the mean, the fit quality, and the size of any one coefficient.',
  },
  {
